...
static bool numaker_usbd_xfer_can_arm(const struct device *dev, uint8_t ep)
{
	...
	if (config->is_hsusbd) {
		 ...
		 */
		if (ep == USB_CONTROL_EP_IN && bi->data) {
			uint32_t evt;
			uint32_t evt_mask = BIT(NUMAKER_USBD_EVENT_CTRL_SETUP_COMPL) |
			...
			evt = k_event_wait(&priv->events, evt_mask, false,
					   K_USEC(NUMAKER_HSUSBD_CTRL_DATA_IN_TOKEN_TIMEOUT_US));
